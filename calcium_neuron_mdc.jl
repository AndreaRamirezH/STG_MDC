...
function Neuron_wHomeo_dynamics(input)
    ...
    od = structural_simplify(od) #make sure it's not a DAE
    tspan = (0.,800.)
...
# Cost functions
#integrate error from half time to end
t0=400.
tf=800.

function redo(p,nom_prob)
    prob = remake(nom_prob; p=p)
    sol = solve(prob, alg(),reltol=1e-11,abstol=1e-11)
    return prob,sol
end

#option 1 compare to nom_ca
loss_nom_ca(sol) = quadgk(t-> abs2(nom_sol(t)[2] - sol(t)[2]),t0,tf)[1]
nom_ca_loss(p)=loss_nom_ca(redo(p,nom_prob)[2])

function nom_ca_loss(p,g)
  g[:] = ForwardDiff.gradient(p) do p
    nom_ca_loss(p)
  end
  return nom_ca_loss(p)
end

#option 2 compare to Ca_tgt
loss_tgt(sol) = quadgk(t-> abs2(Ca_tgt .- sol(t)[2]),t0,tf)[1]
ca_tgt_loss(p) = loss_tgt(redo(p,nom_prob)[2])

function ca_tgt_loss(p,g)
    g[:] = ForwardDiff.gradient(p) do p
      ca_tgt_loss(p)
    end
    return ca_tgt_loss(p)
end
